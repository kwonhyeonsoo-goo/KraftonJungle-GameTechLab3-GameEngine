...
namespace
{
	...
	bool TryParseFloatArray1(const std::string& InLine, float& OutValue)
	{
		...
		std::string Data = InLine.substr(Start + 1, End - Start - 1);
		std::replace(Data.begin(), Data.end(), ',', ' ');

		std::stringstream ss(Data);
		return (ss >> OutValue) ? true : false;
	}
	...
	bool TryParseFloatArray3(const std::string& InLine, FVector& OutValue)
	{
		...
		std::stringstream ss(Data);
		return (ss >> OutValue.X >> OutValue.Y >> OutValue.Z) ? true : false;
	}
	...
	bool TryParseQuotedStringValue(const std::string& InLine, FString& OutValue)
	{
		...
		if (SecondQuote == std::string::npos || SecondQuote <= FirstQuote) return false;
	...
	{
		int ParsedId = 0;
		if (::sscanf_s(InLine.c_str(), "\"%d\" : {", &ParsedId) != 1) return false;
	...
	std::wstring ResolveAssetPath(const std::wstring& InScenePath, const FString& InRelativeAssetPath)
	{
		...
		if (FFileSystem::IsAbsolutePath(AssetPath) && FFileSystem::SafeExists(AssetPath)) return AssetPath;
		...
		for (int Depth = 0; Depth < 6; ++Depth)
		{
			...
			if (FFileSystem::SafeExists(Candidate)) return Candidate;
			if (!FFileSystem::HasParentPath(Cursor)) break;
...
bool FScene::LoadFromFile(ID3D11Device* InDevice, ID3D11DeviceContext* InDeviceContext, const std::wstring& InSceneFilePath)
{
	...
	if (InDevice == nullptr || InDeviceContext == nullptr || InSceneFilePath.empty()) return false;

	const std::wstring SceneFilePath = FFileSystem::GetAbsolutePath(InSceneFilePath);
	FILE* SceneFile = nullptr;
	_wfopen_s(&SceneFile, SceneFilePath.c_str(), L"rb");
	...
	while (FFileSystem::GetLineFromFile(SceneFile, Line))
	{
		...
		{
			std::string TempLine;
			while (FFileSystem::GetLineFromFile(SceneFile, TempLine))
			{
		...
		if (bInCameraBlock)
		{
			if (TrimmedLine == "}," || TrimmedLine == "}") { bInCameraBlock = false; continue; }

			float ScalarValue = 0.0f;
			FVector VectorValue = FVector::ZeroVector;
			if (TrimmedLine.starts_with("\"FOV\"") && TryParseFloatArray1(TrimmedLine, ScalarValue)) InitialCamera.FovDegrees = ScalarValue;
			else if (TrimmedLine.starts_with("\"NearClip\"") && TryParseFloatArray1(TrimmedLine, ScalarValue)) InitialCamera.NearClip = ScalarValue;
			else if (TrimmedLine.starts_with("\"FarClip\"") && TryParseFloatArray1(TrimmedLine, ScalarValue)) InitialCamera.FarClip = ScalarValue;
			else if (TrimmedLine.starts_with("\"Location\"") && TryParseFloatArray3(TrimmedLine, VectorValue)) RawCameraLocation = VectorValue;
			else if (TrimmedLine.starts_with("\"Rotation\"") && TryParseFloatArray3(TrimmedLine, VectorValue)) RawCameraRotation = VectorValue;
		...
		if (bInPrimitiveBlock)
		{
			if (TrimmedLine == "}," || TrimmedLine == "}")
			{
				...
				if (PendingPrimitive.Type != "StaticMeshComp" || PendingPrimitive.MeshAssetPath.empty()) continue;
				...
				if (!SharedMesh || !SharedMesh->IsValid()) continue;
			...
			FVector VectorValue = FVector::ZeroVector;
			FString StringValue;
			if (TrimmedLine.starts_with("\"Location\"") && TryParseFloatArray3(TrimmedLine, VectorValue)) PendingPrimitive.Location = VectorValue;
			else if (TrimmedLine.starts_with("\"Rotation\"") && TryParseFloatArray3(TrimmedLine, VectorValue)) PendingPrimitive.Rotation = VectorValue;
			else if (TrimmedLine.starts_with("\"Scale\"") && TryParseFloatArray3(TrimmedLine, VectorValue)) PendingPrimitive.Scale = VectorValue;
			else if (TrimmedLine.starts_with("\"ObjStaticMeshAsset\"") && TryParseQuotedStringValue(TrimmedLine, StringValue)) PendingPrimitive.MeshAssetPath = StringValue;
			else if (TrimmedLine.starts_with("\"Type\"") && TryParseQuotedStringValue(TrimmedLine, StringValue)) PendingPrimitive.Type = StringValue;
		...
		{
			if (TrimmedLine == "}" || TrimmedLine == "},") { bInPrimitivesBlock = false; continue; }
