...
```powershell
msbuild .\DinoEngine.sln /restore /m /p:Configuration=Debug /p:Platform=x64
```

## 주의사항

- `Engine\packages.config`의 DirectXTK가 `/restore`로 복원되어야 합니다.
- 폴더명에 `+`가 있으므로 셸 명령이나 자동화에서 경로 전체를 따옴표로 감싸는 편이 안전합니다.
- `ObjViewerDebug` 구성도 존재하지만 검증 기준은 `Debug|x64`입니다.
