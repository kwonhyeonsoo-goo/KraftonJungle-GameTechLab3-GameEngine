...
```powershell
msbuild .\NipsEngine.sln /restore /m /p:Configuration=Debug /p:Platform=x64
```

## 주의사항

- `NipsEngine\packages.config`의 DirectXTK가 `/restore`로 복원되어야 합니다.
- `ObjViewer`와 `Release` 구성도 존재하지만 현재 확인된 구성은 `Debug|x64`입니다.
