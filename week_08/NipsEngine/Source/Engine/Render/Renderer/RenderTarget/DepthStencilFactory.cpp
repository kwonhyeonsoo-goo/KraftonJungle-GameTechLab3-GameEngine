...
FDepthStencilResource FDepthStencilFactory::CreateDepthStencilViewCubemapArray(ID3D11Device* Device, uint32 InWidth, uint32 InHeight, uint32 CubeCount)
{
	return FDepthStencilBuilder().SetSize(InWidth, InHeight).WithStencil().WithSRV().AsCubemapArray(CubeCount).Build(Device);
}
