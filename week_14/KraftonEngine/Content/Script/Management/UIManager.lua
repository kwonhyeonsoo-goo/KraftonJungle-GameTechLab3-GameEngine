...
function UIManager:ApplyCutSceneLetterbox(widget, alpha)
    ...
    self:SetElementAlpha(widget, CUTSCENE_LETTERBOX_TOP_ID, alpha)
    self:SetElementAlpha(widget, CUTSCENE_LETTERBOX_BOTTOM_ID, alpha)
end
...
function UIManager:SetScopeTelemetry(payload)
    ...
    if type(payload.wind_degrees) == "number" and type(payload.wind_mps) == "number" then
        wind_text = string.format("%03d deg  %.1f m/s", math.floor(payload.wind_degrees + 0.5) % 360, payload.wind_mps)
    end
    ...
    local telemetry_values = {
        ...
        scopeWindValue = tostring(wind_text),
