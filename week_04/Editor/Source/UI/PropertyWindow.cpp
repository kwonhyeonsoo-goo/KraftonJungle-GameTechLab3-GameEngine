		...
		if (SelectedActor)
		{
			...
			if (ImGui::CollapsingHeader("Material", ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				for (UActorComponent* Component : SelectedActor->GetComponents())
				{
					...
					if (Component->IsA(UObjComponent::StaticClass()))
					{
						UObjComponent* ObjComp = static_cast<UObjComponent*>(Component);
						FStaticMesh* Mesh = ObjComp->GetStaticMesh();
						if (!Mesh) continue;
						...
						{
							FMaterial* CurrentMat = ObjComp->GetMaterialBySlot(i);
							...
							if (ImGui::Combo(Label.c_str(), &CurrentIndex, MatNames.data(), static_cast<int>(MatNames.size())))
							{
								...
								ObjComp->SetMaterialSlot(i, NewMat);
							}
						}
					}
				}
